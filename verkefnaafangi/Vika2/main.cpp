...
int main()
{
    ...
    db.open();
    QSqlQuery query(db);
    query.exec("CREATE TABLE Scientists(id integer primary key, firstName text, lastName text, sex text, YoB int, YoD int)");
    ...
    subject.push_back(Scientists(id, firstName, lastName, sex, YoB, YoD));
    }
